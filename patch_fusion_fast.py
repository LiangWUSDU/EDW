...
flow_weight = np.exp(-np.array(flow_weight))
flow_weight = flow_weight[...,np.newaxis]
flow_weight = np.concatenate([flow_weight,flow_weight,flow_weight],axis=-1)
##
xx = np.arange(192)
yy = np.arange(160)
zz = np.arange(160)
...
for i in range(len(moving_image_patch)):
	...
	LOC[i][2].start:LOC[i][2].stop, :] += pred_flow[0, :, :, :,:] * flow_weight
	overlap_weight_flow[LOC[i][0].start:LOC[i][0].stop,LOC[i][1].start:LOC[i][1].stop,
	LOC[i][2].start:LOC[i][2].stop, :] += flow_weight
